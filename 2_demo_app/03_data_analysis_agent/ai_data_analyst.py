...
def preprocess_and_save(file):
    try:
        # 1. 根据后缀读文件 Read the uploaded file into a DataFrame
        if file.name.endswith('.csv'):
            # na_values：把 'NA'、'N/A' 等字符串也识别成 NaN
        ...
        # 2. 字符串列强制转 str，并把 " 转成 ""
        # select_dtypes(['object'])：选出字符串列
        for col in df.select_dtypes(include=['object']):
            df[col] = df[col].astype(str).replace({r'"': '""'}, regex=True)

        # 3. 尝试自动转换日期和数值列 （智能类型推断，提升 DuckDB 查询体验）
        for col in df.columns:
            if 'date' in col.lower():  # 列名含“date” → 转 datetime
                df[col] = pd.to_datetime(df[col], errors='coerce')  # errors='coerce'：非法日期转 NaT，不报错
            elif df[col].dtype == 'object':
                try:
                    df[col] = pd.to_numeric(df[col]) #  失败就保持 object（比如混杂文本的列）
                except (ValueError, TypeError):
                    # Keep as is if conversion fails
                    pass

        # 4. 创建临时 CSV 文件（关键！）
        # 创建临时文件，程序结束不自动删（因为 DuckDB 需要路径持久存在）
        with tempfile.NamedTemporaryFile(delete=False, suffix=".csv") as temp_file:
            temp_path = temp_file.name
            # Save the DataFrame to the temporary CSV file with quotes around string fields
            # quoting=csv.QUOTE_ALL：所有字段都强制加双引号
            df.to_csv(temp_path, index=False, quoting=csv.QUOTE_ALL)

        return temp_path, df.columns.tolist(), df  # 返回：临时文件路径  列名列表  原始DataFrame
...
if uploaded_file is not None and "openai_key" in st.session_state:
    ...
    if temp_path and columns and df is not None:
        ...
        # 把临时 CSV 加载成 DuckDB 表 uploaded_data
        ...
        # system_message：明确告诉 Agent 用哪个表、生成 SQL、输出简洁
